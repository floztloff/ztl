...
function CoursesTab() {
  ...
  const doGenerate = async (recs) => {
    setBusy(true); setErr("");
    try {
      var r = recs || recipes || await loadRecipes();
      var lines = await collectLines(r);
      var base = items || [];
      var prevChecked = {};
      base.forEach(function(it) { if (it.checked && it.name) prevChecked[it.name] = true; });
      var manual = base.filter(function(it) { return it.manual; });
      var consolidated = [];
      if (lines.length > 0) {
        try {
          consolidated = await aiShoppingList(lines);
          console.log("IA consolidated:", consolidated.length, "articles");
        } catch (e) {
          console.warn("aiShoppingList failed, fallback local:", e);
          consolidated = localAggregate(lines);
          setErr("Liste générée localement (IA indisponible).");
        }
      }
      if (lines.length > 0 && consolidated.length === 0) {
        consolidated = localAggregate(lines);
      }
      var generated = consolidated.map(function(c) {
        var name = norm(c.item);
        var text = c.qty ? c.qty + " " + c.item : c.item;
        return { id: uid(), name: name, text: text, checked: !!prevChecked[name], manual: false };
      });
      persist([].concat(generated, manual));
      if (lines.length === 0) setErr("Aucune recette planifiée à venir. Ajoute des repas dans le Programme.");
    } catch (e) {
  ...
  useEffect(() => {
    (async () => {
      var r = await loadRecipes();
      var s = await store.get("shopping");
      var stored = Array.isArray(s) ? s : [];
      setItems(stored);
      if (!stored.length) doGenerate(r);
    })();
  }, []);

  const toggle = (id) => persist((items || []).map(function(it) { return it.id === id ? Object.assign({}, it, { checked: !it.checked }) : it; }));
  const del = (id) => persist((items || []).filter(function(it) { return it.id !== id; }));
  const startEdit = (it) => { setEditId(it.id); setEditText(it.text); };
  const saveEdit = () => { var t = editText.trim(); if (t) persist((items || []).map(function(it) { return it.id === editId ? Object.assign({}, it, { text: t, name: norm(t) }) : it; })); setEditId(null); setEditText(""); };
  const addManual = () => { var t = newItem.trim(); if (!t) return; persist((items || []).concat([{ id: uid(), name: norm(t), text: t, checked: false, manual: true }])); setNewItem(""); };
  const clearChecked = () => persist((items || []).filter(function(it) { return !it.checked; }));

  if (!items) return <div style={{ color: C.mut, fontSize: 13, paddingTop: 8 }}>Chargement…</div>;
  var sorted = [].concat(items).sort(function(a, b) { return a.checked === b.checked ? 0 : a.checked ? 1 : -1; });
  var remaining = items.filter(function(it) { return !it.checked; }).length;
  ...
  return (
    <>
      ...
      <button onClick={async function(){try{await store.syncFromCloud();var u=window._ztlUser&&window._ztlUser.id;var raw="";for(var i=0;i<localStorage.length;i++){var k=localStorage.key(i);if(k.indexOf("log:")===0)raw+=k+"="+localStorage.getItem(k).slice(0,150)+"\n";}alert(raw||"AUCUN log dans localStorage");}catch(e){alert("Err: "+e.message);}}} style={{marginTop:4,width:"100%",background:"#eee",border:"1px solid #999",borderRadius:8,padding:"5px",fontSize:10,cursor:"pointer"}}>🔬 Debug syncFromCloud</button>
      ...
      <button onClick={async function(){var u=window._ztlUser&&window._ztlUser.id;if(!u||!window.ZTLDb)return;var dbg=[];for(var w=0;w<4;w++){var days=weekDaysFrom(w);for(var d=0;d<days.length;d++){var dk=days[d];try{var raw=await window.ZTLDb.getUserData(u,"plan:"+dk);if(typeof raw==="string")raw=JSON.parse(raw);if(raw&&raw.meals&&raw.meals.length)dbg.push(dk+": "+raw.meals.join(","));}catch(e){}}}alert(dbg.length?dbg.join("\n"):"Aucun repas dans aucune semaine");}} style={{width:"100%",background:C.mint,border:"1px solid "+C.teal,borderRadius:10,padding:"6px",fontSize:11,cursor:"pointer",marginBottom:12}}>🔍 Lister tous les repas (Supabase)</button>
            ...
            {sorted.map(function(it, i) {
              return (
                <div key={it.id} style={{ display: "flex", alignItems: "center", gap: 11, padding: "11px 0", borderBottom: i < items.length - 1 ? "1px solid " + C.line : "none" }}>
                  <button onClick={function() { toggle(it.id); }} style={{ width: 22, height: 22, borderRadius: 6, border: "2px solid " + (it.checked ? C.good : C.line), background: it.checked ? C.good : "transparent", cursor: "pointer", flexShrink: 0 }}>
                    {it.checked && <span style={{fontSize:13,lineHeight:1,color:C.bg}}>✅</span>}
                  </button>
                  {editId === it.id ? (
                    <>
                      <input autoFocus value={editText} onChange={function(e) { setEditText(e.target.value); }} onKeyDown={function(e) { if (e.key === "Enter") saveEdit(); if (e.key === "Escape") { setEditId(null); setEditText(""); } }} style={{ flex: 1, boxSizing: "border-box", background: C.bg, border: "1px solid " + C.teal, color: C.text, borderRadius: 8, padding: "7px 9px", fontSize: 14 }} />
                    ...
                    <>
                      ...
                      <button onClick={function() { startEdit(it); }} style={{ background: "none", border: "none", color: C.mut, cursor: "pointer", padding: 4 }}><span style={{fontSize:15,lineHeight:1}}>✏️</span></button>
                      <button onClick={function() { del(it.id); }} style={{ background: "none", border: "none", color: C.mut, cursor: "pointer", padding: 4 }}><span style={{fontSize:15,lineHeight:1}}>🗑️</span></button>
                    </>
                  )}
                </div>
              );
