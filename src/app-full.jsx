...
function ProgramTab() {
  ...
  return (
    <>
      ...
      <div style={{marginTop:10}}>
        <button onClick={async function(){var u=window._ztlUser&&window._ztlUser.id;if(!u)return alert("Pas connecté");try{var p=await window.ZTLDb.getUserData(u,"plan:2026-07-08");alert(p?JSON.stringify(p,"",2):"AUCUNE donnée pour le 8 juillet dans Supabase");}catch(e){alert("Erreur: "+e.message);}}} style={{fontSize:10,padding:"4px 8px",background:C.emberSoft,border:"1px solid "+C.ember,borderRadius:6,cursor:"pointer",marginRight:5}}>🔬 Supabase: plan 08/07</button>
        <button onClick={function(){var dbg=[];for(var i=0;i<localStorage.length;i++){var k=localStorage.key(i);if(k.indexOf("plan:")===0){try{var v=JSON.parse(localStorage.getItem(k));dbg.push(k.slice(5)+": meals="+(v.meals||[]).length+" sessions="+(v.sessions||[]).length);}catch(e){dbg.push(k.slice(5)+": err");}}}alert(dbg.length?dbg.join("\n"):"AUCUNE clé plan: dans localStorage");}} style={{fontSize:10,padding:"4px 8px",background:C.mint,border:"1px solid "+C.teal,borderRadius:6,cursor:"pointer"}}>🔬 localStorage</button>
      </div>
