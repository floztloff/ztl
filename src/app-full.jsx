...
function CoursesTab() {
  ...
  return (
    <>
      ...
      <button onClick={async function(){try{await window.store.syncFromCloud();var raw="";for(var i=0;i<localStorage.length;i++){var k=localStorage.key(i);if(k.indexOf("log:")===0)raw+=k+"="+localStorage.getItem(k).slice(0,150)+"\n";}setErr(raw||"AUCUN log dans localStorage");}catch(e){setErr("Err: "+e.message);}}} style={{marginTop:4,width:"100%",background:"#eee",border:"1px solid #999",borderRadius:8,padding:"5px",fontSize:10,cursor:"pointer"}}>🔬 Debug syncFromCloud</button>
